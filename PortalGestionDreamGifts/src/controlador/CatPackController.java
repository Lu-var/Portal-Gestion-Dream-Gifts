 ...
 * @author luvar
 */
public class CatPackController {
    
    CatPackManager manager = new CatPackManager(); 
    
    public void agregarCategoria(Maestro vista){
        String nombre = vista.getTxtCatPack().getText();
    ...
    public void showAll(Maestro master){
        ...
        JTable tabla = master.getTablaCatPack();
...
//            }
            try {
                input = JOptionPane.showInputDialog(null, "Nueva Categoria", fila.get(1));
                nombre = (String)input;
                if(nombre.isEmpty()){
